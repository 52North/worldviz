...
public class CsvReaderForConnectionMap {
     ...
     *
     * @param csvFilePath File path and name
     * @return Feature-net
     */
    public WvizUniversalFeatureNet readFromFile(String csvFilePath, char csvSeperator) {
	...
	/* parse csv-file --> load connections for each country of a row
          (Map<countryCode,List<Connection+Weight>) weight must be taken care of
           maybe some consistency checks
        */
        try {

            if (logger.isInfoEnabled()) {
                logger.info("Trying to parse the CSV-file {} to create a FeatureNet from it.",csvFilePath);
            }
            ...
            while (csvReader.readRecord()) {
                ...
                String fromNodeCountryCode = csvReader.get(this.countryNameColumnHeader);

                // add fromFeatureNode to the list of featureNodes
                VgFeature fromFeatureNode = this.countryCodeSceneNodeMap.get(fromNodeCountryCode);
		    ...
		    /*if nextDataEntry is != 0 then a new connection has been found!
                     thus, if nextDataEntry equals 0, this matrix cell can be skipped
                    ...
                    */
                    if (nextDataEntry.equalsIgnoreCase(this.zeroValue)) {
                        ...
                        double connectionWeight = Double.parseDouble(nextDataEntry);
                        // now link that connection weight to the corresponding node!
                        String toNodeCountryCode = csvReader.getHeader(indexDataColumn);
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug("New Relation constructed: {}", fromToRelation);
                        }
                    }
                }// end for-loop
            }// end while
	    ...
	    // --> construct FeatureNet
            ...
            if (logger.isErrorEnabled()) {
                logger.error("An error (FileNotFound-Exception) occured during parsing the CSV-file {}.",csvFilePath, e);
            }
        }
        catch (IOException e) {
            if (logger.isErrorEnabled()) {
                logger.error("An IO-Exception occured during parsing the CSV-file {}.",csvFilePath, e);
            }
        }
    ...
    private void loadCountriesPointDataset() {
        ...
        DatasetLoader countriesPointLoader = new DatasetLoader(this.pathToCountriesPointFile);

        try {
            XmlDataset countriesPointDataset = countriesPointLoader.loadDataset();

            List<VgAttrFeature> features = countriesPointDataset.getFeatures();

	    // transform this list of features into a map with key=ISO3166-countryCode, value=feature
            this.countryCodeSceneNodeMap = createMapFromFeatureList(features);

            if (this.countryCodeSceneNodeMap == null || this.countryCodeSceneNodeMap.size() == 0) {
                if (logger.isWarnEnabled()) {
                    logger.warn("Loading the CountriesPoint-file at {} failed.",this.pathToCountriesPointFile);
                }
            }
            ...
            if (logger.isErrorEnabled()) {
                logger.error("Failed to load the dataset {}.", this.pathToCountriesPointFile, e);
            }
        }

    }

    private Map<String, VgFeature> createMapFromFeatureList(List<VgAttrFeature> features) {

        Map<String, VgFeature> countryCodeSceneNodeMap = new HashMap<String, VgFeature>(features.size());

        for (VgAttrFeature feature : features) {
            String isoCountryCode = (String) feature.getAttributeValue(countriesPointIsoCodeHeader);
            String countryName = (String) feature.getAttributeValue(countriesPointNameHeader);
            double longitude = Double.parseDouble((String) feature.getAttributeValue(countriesPointLongitudeHeader));
            double latitude = Double.parseDouble((String) feature.getAttributeValue(countriesPointLatitudeHeader));
            ...
            ((GmAttrFeature) node).addAttribute(labelPropertyName, "String",countryName);
