...
public class MappersDemo {
	...
	private void computeWidthMapping(Collection<VgRelation> relations) {
		...
		for (VgRelation vgRelation : relations) {
			...
			System.out.println("Weight '" + weight + "' was mapped to width '"
					+ width + "'.");
		}
	...
	private void colorRelations(Collection<VgRelation> relations) {
		 ...
		 */
		double[] inputWeights = new double[] { 0, 20, 50, 100, 500, 3000 };
		T3dColor[] outputColors = new T3dColor[] { new T3dColor(0, 1, 0),
				new T3dColor(0.25f, 0.75f, 0), new T3dColor(0.5f, 0.5f, 0),
				new T3dColor(0.75f, 0.25f, 0), new T3dColor(1, 0, 0),
				new T3dColor(0, 0, 1) };
		...
		for (VgRelation vgRelation : relations) {
			...
			System.out.println("Weight '" + weight
					+ "' was mapped to color '[RED:" + red + ", GREEN:" + green
					+ ", BLUE:" + blue + "]'.");
		}
	...
	private void colorAttrFeatures(List<VgAttrFeature> attrFeatures) {
		...
		// true sets linear interpolation to true!
		colorMapper4AttrFeatures.setPalette(inputNumberOfEdges, outputColors,
				true);

		for (VgAttrFeature attrFeature : attrFeatures) {
			Object numberOfRelations = attrFeature
					.getAttributeValue(this.ATTRIBUTE_NAME);
			...
			T3dColor color = (T3dColor) coloredAttrFeature
					.getAttributeValue(colorAttributeName);
	...
	private List<VgAttrFeature> generateAttrFeatures(
			WvizUniversalFeatureNet featureNet) {
		// extract nodes
		Collection<VgFeature> features = featureNet.getFeatures();

		List<VgAttrFeature> attrFeatures = new ArrayList<VgAttrFeature>(
				features.size());
		...
		for (VgFeature vgFeature : features) {
			...
			newAttrFeature.addAttribute(this.ATTRIBUTE_NAME,
					Integer.class.toString(), numberOfEdges);
	...
	private WvizUniversalFeatureNet generateFeatureNet() {
		...
		try {
			wvizUniversalFeatureNet = pajekReader
					.readFromFile(RelativePaths.PAJEK_FLOWS_OF_TRADE_NET);
		} catch (PajekException e) {
