...
public class IndiaTreeCover2013 {

	private static String attributeName = "Tree Cover - Area";
	private static String dataXML = RelativePaths.INDIA_TREE_COVER_2013_XML;
	private static String shapeFilePath = RelativePaths.EARTH_SUB_FOLDER + "India_States/India_Admin2_without_Telangana_simplifiedBorders.shp";
	private static String joinHeader = "NAME";
	private static String outputFile = "test/IndiaTreeCover.x3d";
	...
	public static void main(String[] args) {
		...
		XmlDataset indiaTreeCover = null;

		DatasetLoader countryCodeTest = new DatasetLoader(dataXML,
				shapeFilePath, joinHeader);

		try {
			indiaTreeCover = countryCodeTest.loadDataset();
		} catch (Exception e) {
			e.printStackTrace();
		}

		List<VgAttrFeature> features = indiaTreeCover.getFeatures();
...
//		 outputFile);
		...
		for (VgAttrFeature coloredExtrudedFeature : extrudedAndColoredCountries) {
